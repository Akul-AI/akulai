...
class Calender_skill():
    ...
    def add_event(self, akulai:AI)->bool:
        akulai.say("What is the name of the event")
        try:
            event_name = akulai.listen()
            akulai.say("When is this event?")
            event_begin = akulai.listen()
            event_isodate = dateparser.parse(event_begin).strftime("%Y-%m-%d %H:%M:%S")
            akulai.say("What is the event description?")
            event_description = akulai.listen()
            ...
            return False
    def remove_event(self, akulai:AI)->bool:
        akulai.say("What is the name of the event you want to remove?")
        try:
            event_name = akulai.listen()
            try:
                self.calendar.remove_event(event_name=event_name)
                akulai.say("Event removed successfully")
                self.calendar.save()
                return True
            except:
                akulai.say("Sorry I could not find the event",event_name)
                return False
        except:
            print("opps there was an error")
            return False

    def list_events(self, period, akulai:AI)->bool:

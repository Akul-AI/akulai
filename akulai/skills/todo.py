...
def add_todo(akulai:AI)->bool:
    item = Item()
    akulai.say("Tell me what to add to the list")
    try:
        item.title = akulai.listen()
        todo.new_item(item)
...
def list_todos(akulai:AI):
    if len(todo) > 0:
        akulai.say("Here are your to do's")
        for item in todo:
            akulai.say(item.title)
    else:
        akulai.say("The to do list is empty!")

def remove_todo(akulai:AI)->bool:
    akulai.say("Tell me which item to remove")
    try:
        item_title = akulai.listen()
